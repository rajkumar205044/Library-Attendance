...
if(isset($_POST['print'])){
	...
	$to=$_POST["to"];
    $depts=$_POST["dept"];
    $query ="select * from logs where date between '".$from."' and '".$to."'";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if (in_array($row_students['department'], $depts)) {
    ...
    if ($from==$to)
        header('Content-Disposition: attachment; filename="'.$from_date->format('Ymd').'.csv"');
    else
header('Content-Disposition: attachment; filename="'.$from_date->format('Ymd').'-'.$to_date->format('Ymd').'.csv"');
echo $csv_data;
exit;
    }
?>
...
<div class="input">
  ...
  <input type="date" id="fname" name="from" required><br><br>
  <label for="lname" style="color:white;">To:</label>
  <input type="date" id="lname" name="to" required><br><br>
</div>
<div class="checkbox">
<?php
...
$result=mysqli_query($conn,$query);
while($row=mysqli_fetch_assoc($result)){
	echo "  <input type='checkbox' id='checkbox' name='dept[]' value='".$row['id']."'>
  <label style='color:white;' >".$row['name']." ". $row['year'] ."</label><br><br>";
}
...
<button class="print" type="submit" id="downloadBtn" name="print"><i class="fa-solid fa-print"></i>print</button>
</form>
        </article> 
        <form method="post">
        <aside>
        ...
        <nav class="menu">  
            ...
            <button style="width: 100%;" class="current" name="date"><h2><i class="fa-solid fa-calendar-days"></i> Print Attendance</h2></button>
            <button style="width: 100%;" name="database"><h2><i class="fa-solid fa-database"></i> DataBase</h2></button>
        </nav></form>
        <div class="user">
        <p><?php echo $userid?></p>
        </div>
